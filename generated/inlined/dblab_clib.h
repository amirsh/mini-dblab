...
inline unsigned long long timeval_subtract(struct timeval *result, struct timeval *t2, struct timeval *t1) {
...
inline char* strntoi_unchecked(char *s, numeric_int_t* num) {
	int n = 0;
	int sign = 1;
	if (*s == '-') {
		s++;
...
inline char* strntod_unchecked(char *s, double* num)
{
	int n = 0;
	int d = 0;
	int dlen = 0;
...
inline char* strntodec_unchecked(char *s, decimal_t* num, int scale)
{
	int n = 0;
	int d = 0;
	int dlen = 0;
...
inline char** tokenizeString(char *sentence) {
...
inline struct timeval tic() {
	struct timeval res = (struct timeval){0};
	gettimeofday(&res, NULL);
	return res;
}

inline unsigned long long toc(struct timeval* start) {
